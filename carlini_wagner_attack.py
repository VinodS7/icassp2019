...
class CarliniWagnerAttack():
    ...
    def build_attack(self):
        ...
        self.const = tf.Variable(hparams.initial_const,name='qq_const')
        ...
        features = inputs.compute_features(self.new_input)
        self.logits = logits = self.model.get_logits(features)
        ...
        other = tf.reduce_max((11-self.labels)*self.output-self.labels+10000,1)
        if hparams.targeted:
    ...
    def attack(self,input_audio,labels,is_first=True,offset=10):
        ...
        sess.run([self.init,tf.variables_initializer([self.const])],feed_dict={self.delta_shape:audio_shape})
        ...
        while(True):
            now = time.time()
            l1,l2,l,op=sess.run([self.loss1,self.loss2,self.loss,self.model.get_probs()],feed_dict={self.labels:labels,self.offset:offset}) 
            ...
            print(l2,op) 
            if(hparams.targeted):
                if(np.argmax(op) == np.argmax(labels) and l2>25):
                    snr = l2
